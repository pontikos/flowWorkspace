...
setMethod("getNodes","GatingHierarchy",function(x,y=NULL,tsort=FALSE,isPath=FALSE,...){
#			browser()
			if(!tsort){
				nodeNames<-x@nodes
			}else{
				nodeNames<-RBGL::tsort(x@tree)
			}
			
			nodePaths<-unlist(lapply(nodeNames,function(y)mget(c("fjName"),nodeData(x@tree,y,"metadata")[[1]])[[1]]))
